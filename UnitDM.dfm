object DM: TDM
  ...
  object sqlEAcesso: TFDQuery
    Connection = Conexao
    SQL.Strings = (
      'SELECT --USG_CODIGO CODIGO'
      '      --USU_CODIGO COD_USU'
      '      COUNT(D.USU_CODIGO_GRUPO) GRUPO_COD'
      '      ,D.USU_CODIGO_GRUPO'
      '      ,D.GRUPO_NOME GRUPO_NOME'
      '       '
      '  FROM (SELECT USG.*,'
      '               (SELECT USU_NOME '
      '                  FROM :BASE.USUARIO'
      '                 WHERE USU_CODIGO = USG.USU_CODIGO) AS USU_NOME,'
      '               (SELECT USU_NOME'
      '                  FROM :BASE.USUARIO'
      
        '                 WHERE USU_CODIGO = USG.USU_CODIGO_GRUPO) AS GRU' +
        'PO_NOME'
      '          FROM :BASE.USUARIO_GRUPO USG) D'
      ' WHERE USU_CODIGO IN (SELECT USU_CODIGO'
      '                        FROM :BASE.USUARIO'
      
        '                       WHERE USU_NOME IN (UPPER(:NOME1), UPPER(:' +
        'NOME2)))'
      ' GROUP BY USU_CODIGO_GRUPO, GRUPO_NOME'
      'HAVING COUNT(USU_CODIGO_GRUPO) < 2')
    Left = 168
    Top = 152
    ParamData = <
      item
  ...
  object sqlEBase: TFDQuery
    ...
    SQL.Strings = (
      ...
      'WHERE A.USU_NOME = UPPER(:NOME1)'
      'OR   A.USU_NOME = UPPER(:NOME2)'
      'GROUP BY A.BASE'
